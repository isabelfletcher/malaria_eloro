...
## Here we test for the best monthly time lags for minimum temperature in spatiotemporal models of malaria incidence
...
tmin_lag0 <- scale(data_pf$tmin, center = TRUE, scale = TRUE)[,1]
...
df_inla_pf <- data.frame(y, prcp, tmin_lag0, tmin_lag1,
                         tmin_lag2, tmin_lag3, 
                         t1, t2, s1, s2, 
                         total_poverty, urban, int_per)


lags <- c("tmin_lag0", "tmin_lag1", "tmin_lag2", "tmin_lag3")
...
for (i in 1:4) {
  ...
  tmin <- scale(df_inla_pf[,grep(lags[i], names(df_inla_pf), value=TRUE)], 
  ...
  tmin_lags_l  <- rbind(tmin_lags_l, df_l)
  tmin_lags_nl <- rbind(tmin_lags_nl, df_nl)
  
}

data_l  <- tmin_lags_l
data_nl <- tmin_lags_nl
...
##################################
# Add time lags and scale variables
tmin_lag0 <- scale(data_pv$tmin, center = TRUE, scale = TRUE)[,1]
...
data_l  <- rbind(data_l, tmin_lags_l)
data_nl <- rbind(data_nl, tmin_lags_nl)
