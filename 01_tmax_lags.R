...
## Here we test for the best monthly time lags for maximum temperature in spatiotemporal models of malaria incidence
...
tmax_lag0 <- scale(data_pf$tmax, center = TRUE, scale = TRUE)[,1]
...
df_inla_pf <- data.frame(y, prcp, tmax_lag0, tmax_lag1,
                         tmax_lag2, tmax_lag3, 
                         t1, t2, s1, s2, 
                         total_poverty, urban, int_per)


lags <- c("tmax_lag0", "tmax_lag1", "tmax_lag2", "tmax_lag3")
...
for (i in 1:4) {
  ...
  tmax <- scale(df_inla_pf[,grep(lags[i], names(df_inla_pf), value=TRUE)], 
  ...
  tmax_lags_l  <- rbind(tmax_lags_l, df_l)
  tmax_lags_nl <- rbind(tmax_lags_nl, df_nl)
  
}

data_l  <- tmax_lags_l
data_nl <- tmax_lags_nl
...
##################################
# Add time lags and scale variables
tmax_lag0 <- scale(data_pv$tmax, center = TRUE, scale = TRUE)[,1]
...
data_l  <- rbind(data_l, tmax_lags_l)
data_nl <- rbind(data_nl, tmax_lags_nl)

write.csv(data_l,  file = "supplementary/table_s2_tmax_lags_l.csv")
write.csv(data_nl, file = "supplementary/table_s3_tmax_lags_nl.csv")
